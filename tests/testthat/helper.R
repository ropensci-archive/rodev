...
scoped_temporary_thing <- function(dir = file_temp(pattern = pattern),
  ...
  if (identical(env, globalenv())) {
    ...
    withr::defer(usethis::proj_set(old_project, force = TRUE, quiet = TRUE), envir = env)
  }

  switch(
    thing,
    package = capture_output(usethis::create_package(dir, rstudio = rstudio, open = FALSE)),
    project = capture_output(usethis::create_project(dir, rstudio = rstudio, open = FALSE))
  )
  invisible(dir)
}
